import{a as D,r as h,j as e,h as B,aG as U,n as g,aH as _}from"./index-3pZLK3h4.js";import{c as b}from"./pii-BcFV6mqh.js";function u(a){if(!a)return"";const l=a instanceof Date?a:new Date(a);return Number.isNaN(l.getTime())?String(a):l.toLocaleString()}function O(a,l){const p=new Blob([JSON.stringify(l,null,2)],{type:"application/json"}),m=URL.createObjectURL(p),c=document.createElement("a");c.href=m,c.download=a,document.body.appendChild(c),c.click(),c.remove(),URL.revokeObjectURL(m)}function r(a){return String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function F(a){const l=window.open("","_blank","noopener,noreferrer");if(!l){g.error("Popup blocked");return}const p=Array.isArray(a.logs)?a.logs:[],m=b(a)||a.candidateId,c=p.map(i=>`<tr>
          <td>${r(u(i.at))}</td>
          <td>${r(i.stageTag)}</td>
          <td>${r(i.remark)}</td>
          <td>${r(i.actorRole)}</td>
